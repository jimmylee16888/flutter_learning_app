...
class AdProvider {
  ...
  Future<void> initialize({bool forceRefresh = false}) async {
    ...
    if (kAdFallbacks.isNotEmpty) {
      ...
      _cached = AdRenderInfo(banner: NetworkImage(_bustUrl(f.bannerUrl)), onTap: () => _openUrl(f.clickUrl));
      return;
    }
    // 3) 最後的保底，避免完全沒圖
    const fallbackBanner = "asset:assets/images/ad_banner.png";
    _cached = AdRenderInfo(banner: NetworkImage(_bustUrl(fallbackBanner)), onTap: () async {});
  }
  ...
  Future<AdRenderInfo?> _tryFetchRemote({bool forceRefresh = false}) async {
    try {
      ...
      final res = await http.get(adJsonUri, headers: const {'Cache-Control': 'no-cache', 'Pragma': 'no-cache'});
      ...
      return AdRenderInfo(banner: NetworkImage(bustedBanner), onTap: () => _openUrl(clickUrl));
    } catch (_) {
      return null;
    }
  }
