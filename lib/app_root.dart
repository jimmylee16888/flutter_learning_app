...
/// =========================
/// AppRoot：改成 Stateful，開 App 自動同步一次
/// =========================
...
class _AppRootState extends State<AppRoot> {
  bool _autoSyncScheduled = false; // 確保只排一次
  ...
  void _ensureAutoSync(BuildContext context, AuthController auth) {
    ...
    Future.microtask(() async {
      try {
        ...
        // 1️⃣ 啟動時先照你原本規則：server 有 snapshot → 以 server 為主
        await syncService.syncOnAppStart();

        // 2️⃣ 再做「每天一次」的 push / merge（距上次 <24h 會自動跳過）
        await syncService.syncDailyIfNeeded();

        debugPrint('[LibrarySync] auto sync (startup + daily) done');
      } catch (e, st) {
    ...
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        return AnimatedBuilder(
          ...
          builder: (__, ___) {
            // 🔁 每次 auth 狀態變化都會進來一次
            // 一旦變成 isAuthenticated=true，就排一次同步
